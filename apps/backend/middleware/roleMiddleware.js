...
const roleMiddleware = {
  ...
  staffOnly: requireRole(['admin', 'dtam_officer', 'inspector']),

  // Registrar (Document checker)
  registrarOnly: requireRole(['admin', 'dtam_officer']),

  // Finance staff
  financeOnly: requireRole(['admin', 'dtam_officer']), // Assuming dtam_officer handles finance

  // Auditor/Inspector
  auditorOnly: requireRole(['admin', 'inspector']),

  // Farmer/Applicant
  farmerOnly: requireRole(['farmer']),

  // Farmer or Staff (for viewing applications)
  farmerOrStaff: requireRole(['farmer', 'admin', 'dtam_officer', 'inspector']),
};
...
const canAccessApplication = async (req, res, next) => {
  try {
    ...
    if (
      ...
      (application.assignedInspector && application.assignedInspector.toString() === userId.toString())
    ) {
      return next();
    }
