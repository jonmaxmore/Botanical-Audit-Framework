...
async function initializeWorkflowEngines() {
  try {
    ...
    const farmManagementModule = require('./apps/backend/modules/farm-management');
    // Survey engine is now part of survey-system module
    const surveySystemModule = require('./apps/backend/modules/survey-system');
  ...
  try {
    ...
    mainLogger.info(
      ...
      farmManagementRoutes.stack
        ? `✅ ${farmManagementRoutes.stack.length} routes`
        : '❌ NO ROUTES',
    );
