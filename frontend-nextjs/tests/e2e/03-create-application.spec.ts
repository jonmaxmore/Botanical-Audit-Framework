...
test.describe('Create Application Flow', () => {
  ...
  test('TC 2.2.1: Navigate to create application page', async ({ page }) => {
    ...
    // Verify URL changed
    await page.waitForURL(/.*application.*create|create.*application|applications\/new/i, { timeout: 10000 });
  ...
  test('TC 2.2.4: Successfully create application', async ({ page }) => {
    ...
    const successIndicator = 
      ...
      await page.getByText(/success|สำเร็จ|created|บันทึกแล้ว/i).isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test('TC 2.2.6: Retry logic on create application', async ({ page }) => {
    ...
    const hasRetryLogs = consoleMessages.some(msg => 
      msg.includes('retry') || 
      msg.includes('Retry') || 
